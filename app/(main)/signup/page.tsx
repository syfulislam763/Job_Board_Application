        ...
        <div className="bg-white border-[1.5px] border-[#E5E8F0] rounded-3xl px-8 py-9">
          ...
          <div className="grid grid-cols-2 gap-2 bg-[#F5F6FA] p-1.5 rounded-[14px] mb-5.5">
            {(['seeker', 'employer'] as const).map((r) => (
              <button
                ...
                type="button"
                onClick={() => setRole(r)}
                className={`py-2.5 text-[13px] font-bold rounded-[10px] border-none cursor-pointer font-['Plus_Jakarta_Sans',sans-serif] transition-all duration-200 ${
                  role === r
                    ? 'bg-white text-[#4B6BF5] shadow-sm'
                    : 'bg-transparent text-[#6B7589]'
                }`}
              >
          ...
          <p className="text-center text-[13px] text-[#6B7589] mt-5">
            ...
            {onNavigate ? (
              ...
              <Link href="/login" className="font-bold text-[#4B6BF5] hover:text-[#3451D1] transition-colors">
                Sign in
              </Link>
            )}
          </p>
